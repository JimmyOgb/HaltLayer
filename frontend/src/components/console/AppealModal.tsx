...
export const AppealModal: React.FC<AppealModalProps> = ({
  ...
  const { appealIncident, resolveAppeal } = useProtocol();
  ...
  const handleResolve = async (overturn: boolean) => {
    ...
    try {
      await resolveAppeal(
        ...
        overturn,
        resolutionNotes.trim() || (overturn ? "Vulnerability audited and mitigated." : "Exploit verified. Permanent halt upheld.")
      );
      onClose();
    } catch (err: any) {
          ...
          {isHaltAccepted && (
            <form onSubmit={handleFileAppeal} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2 border-t border-white/10">
                ...
                >
                  {isSubmitting ? "Submitting Appeal..." : "Submit Appeal to Council"}
                </button>
              </div>
            </form>
          )}
