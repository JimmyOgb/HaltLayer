...
export const LiveVaultControls: React.FC<LiveVaultControlsProps> = ({ isOpen, onClose }) => {
  const { state, depositToVault, withdrawFromVault, toggleVaultPause } = useProtocol();
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      setActionSuccess(`Deposit submitted: ${tx.slice(0, 12)}...`);
    } catch (err: any) {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      setActionSuccess(`Withdrawal submitted: ${tx.slice(0, 12)}...`);
    } catch (err: any) {
      // In halted state, this provides direct proof
  ...
  const handleTogglePause = async () => {
    ...
    try {
      ...
      setActionSuccess(`Pause state toggle submitted: ${tx.slice(0, 12)}...`);
    } catch (err: any) {
      ...
      <div className="bg-[#0D121D] border border-white/10 rounded-2xl w-full max-w-xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-5 text-xs font-mono">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Withdraw Box (The critical test of circuit breaker defense) */}
            <div className="p-4 rounded-xl border border-white/10 bg-white/[0.02] flex flex-col justify-between">
              <div>
                <span className="font-bold text-white block mb-1">Test Exploit Drain</span>
                <span className="text-gray-400 text-[11px] font-sans block mb-3">
